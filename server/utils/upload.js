...
if (useSupabase) {
  // Use memory storage, then push to Supabase Storage
  storage = multer.memoryStorage();
} else {
  // Local fallback: use disk storage
...
const createUploadMiddleware = (multerInstance) => ({
  ...
  fields: (fieldsArray) => {
    const multerMiddleware = multerInstance.fields(fieldsArray);
    return async (req, res, next) => {
      multerMiddleware(req, res, async (err) => {
        if (err) return next(err);
        if (req.files && useSupabase) {
          try {
            if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_KEY) {
              console.warn('⚠️ SUPABASE_URL atau SUPABASE_SERVICE_KEY belum diset.');
            } else {
              for (const field in req.files) {
                for (let i = 0; i < req.files[field].length; i++) {
                  req.files[field][i].supabaseUrl = await uploadToSupabase(req.files[field][i]);
                }
              }
            }
          } catch (uploadErr) {
            console.error('Supabase upload fields error:', uploadErr.message);
          }
        }
        next();
      });
...
const getFileUrl = (file) => {
  if (!file) return null;
  if (useSupabase) return file.supabaseUrl || null;
