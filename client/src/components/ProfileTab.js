...
const ProfileTab = ({ user }) => {
  ...
  const handleDelete = async (id) => {
    const confirmRes = await showConfirm('Hapus Alamat', 'Apakah Anda yakin ingin menghapus alamat ini?');
    if (!confirmRes.isConfirmed) return;
    try {
      await axios.delete(`/api/addresses/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      showSuccess('Alamat Dihapus', 'Alamat berhasil dihapus.');
      fetchAddresses();
    } catch (err) {
      showError('Hapus Gagal', 'Gagal menghapus alamat. Silakan coba lagi.');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        ...
        showSuccess('Alamat Diubah', 'Perubahan alamat berhasil disimpan.');
      } else {
        await axios.post('/api/addresses', form, { headers: { Authorization: `Bearer ${token}` } });
        showSuccess('Alamat Ditambah', 'Alamat baru berhasil ditambahkan.');
      }
      setShowModal(false);
      fetchAddresses();
    } catch (err) {
      showError('Simpan Gagal', err.response?.data?.message || 'Gagal menyimpan alamat.');
    }
  };
